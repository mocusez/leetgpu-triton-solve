...
# ---------------------------------------------------------------------------
# Kernel 1: per query-block pass over all key blocks.
#   S = Q K^T * scale and dP = dO V^T are computed tile by tile and stored to
#   global scratch (both natural and transposed layout) so the later kernels
#   never recompute the two big matmuls.  Online softmax gives the row max m,
#   the row sum l and delta = sum_j P_ij dP_ij.
#   All shape arguments are runtime values (never constexpr) and are kept
#   padded to multiples of 16 by the host, so exactly ONE compilation of this
#   kernel serves every test case.  N_true (the unpadded N) is only used to
#   mask the statistics and is excluded from JIT specialization.
# ---------------------------------------------------------------------------
...
def _attn_bwd_pre(
    ...
    for start_n in range(0, stride_sm, BLOCK_N):
        ...
        mask_n = offs_n < stride_sm          # padded scratch bound (multiple of 16)
        ...
        # only the statistics must exclude padded key columns
...
# ---------------------------------------------------------------------------
# Kernel 2: dQ.  One program per query block; streams key blocks.
#   Reloads S / dP tiles, rebuilds p = exp(s - m) / l and
#   ds = p * (dp - delta) * scale, then dQ += ds @ K.  Padded key columns
#   contribute exactly zero because the padded rows of K are zero.
# ---------------------------------------------------------------------------
@triton.jit
def _attn_bwd_dq(
...
# ---------------------------------------------------------------------------
# Kernel 3: dK and dV.  One program per key block; streams query blocks.
#   Uses the transposed scratch (ST / DPT) so all global traffic stays
#   coalesced.  Padded query rows contribute zero (padded dO rows are zero);
#   padded key rows produce garbage that the host slices away.
# ---------------------------------------------------------------------------
@triton.jit
def _attn_bwd_dkdv(
...
def _launch_pre(grid, args, M, N_true, D, sm_scale, BLOCK_D):
    for bm, bn in _PRE_CONFIGS:
        try:
            _attn_bwd_pre[grid(bm)](*args, M, N_true, D, sm_scale, BLOCK_M=bm,
...
def _launch_dq(grid, args, M, D, sm_scale, BLOCK_D):
    for bm, bn in _DQ_CONFIGS:
        try:
            _attn_bwd_dq[grid(bm)](*args, M, D, sm_scale, BLOCK_M=bm, BLOCK_N=bn,
...
def _launch_dkdv(grid, args, M, N, D, sm_scale, accumulate, BLOCK_D):
    for bn, bm in _DKDV_CONFIGS:
        try:
            _attn_bwd_dkdv[grid(bn)](*args, M, N, D, sm_scale, accumulate, BLOCK_N=bn,
...
def solve(Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor, dO: torch.Tensor,
    ...
    # Pad every shape to a multiple of 16.  Together with runtime (non
    # constexpr) shape arguments this keeps one single JIT specialization key
    # for every test case, so each kernel is compiled exactly once.  Padding
    # is free for the performance case (8192 / 4096 / 128 are multiples of 16).
    ...
    for m0 in range(0, M2, m_chunk):
        ...
        _launch_pre(lambda bm: (triton.cdiv(mc, bm),), pre_args, mc, N, d2, sm_scale, BLOCK_D)

        dq_args = (Kp, S, DP, Mp, Lp, Deltap, dq_c,
                   Kp.stride(0), S.stride(0), DP.stride(0), dq_c.stride(0))
        _launch_dq(lambda bm: (triton.cdiv(mc, bm),), dq_args, mc, d2, sm_scale, BLOCK_D)
        ...
        _launch_dkdv(lambda bn: (triton.cdiv(N2, bn),), dkdv_args, mc, N2, d2, sm_scale,
                     1 if (multi and m0 > 0) else 0, BLOCK_D)
