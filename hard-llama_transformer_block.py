...
def solve(x, output, weights, cos, sin, seq_len):
    ...
    with dev_ctx:
        ...
        _stage("5-outproj-gemm", sync, lambda: _gemm_nt_kernel[(triton.cdiv(T, BM), D_MODEL // BN)](
        ...
        # 6) RMSNorm 2
        _stage("6-rmsnorm2", sync, lambda: _rmsnorm_kernel[(T,)](
            x1, w, OFF_W2, norm2, N=D_MODEL, EPS=EPS, BLOCK=512, num_warps=4))

        # 7) fused gate|up projection -> (T,2816)
        _stage("7-gateup-gemm", sync, lambda: _gemm_nt_kernel[(triton.cdiv(T, BM), GU_DIM // BN)](
            norm2, w, OFF_WGU, gu, T, GU_DIM, D_MODEL,
            BLOCK_M=BM, BLOCK_N=BN, BLOCK_K=BK, num_warps=NW))

        # 8) SwiGLU: silu(gate) * up -> (T,1408)
        _stage("8-silu-mul", sync, lambda: _silu_mul_kernel[(T, triton.cdiv(FFN_HIDDEN, 512))](
            gu, ffn, N=FFN_HIDDEN, gu_stride=GU_DIM, BLOCK_N=512, num_warps=4))

        # 9) down projection, then residual: out = x1 + ffn @ W_down^T
        _stage("9-down-gemm", sync, lambda: _gemm_nt_kernel[(triton.cdiv(T, BM), D_MODEL // BN)](
            ffn, w, OFF_WD, ffn_out, T, D_MODEL, FFN_HIDDEN,
            BLOCK_M=BM, BLOCK_N=BN, BLOCK_K=BK, num_warps=NW))
        _stage("9b-residual-add", sync, lambda: _add_kernel[add_grid](
...
if __name__ == "__main__":
    ...
    torch.manual_seed(0)
    assert torch.cuda.is_available(), "needs a CUDA GPU (e.g. Tesla T4)"
    dev = "cuda"
